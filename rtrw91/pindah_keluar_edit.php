<?php require('header-admin.php'); require('../koneksi.php'); error_reporting(0); $idpindahkeluar = $_GET['idpindahkeluar'];
  $query = mysqli_query($kon, "SELECT * FROM pindah_keluar INNER JOIN pemohon ON pindah_keluar.idpemohon = pemohon.idpemohon WHERE idpindahkeluar = '$idpindahkeluar'");
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark"><i class="far fa-calendar-minus"></i> Pembuatan SKPK</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active"><i class="far fa-calendar-minus"></i> Pembuatan SKPK</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-3 col-sm-12">
            ...
            <div class="card card-widget">
              <div class="card-header">
                <b class="username">Surat Keterangan Pindah</b>
...
<?php 
  ...
  if (isset($_POST['upfile1'])) {
        ...
        $query = mysqli_query($kon, "UPDATE pindah_keluar SET file1 = '$namabaru' WHERE idpindahkeluar = '$idpindahkeluar'");
        if($query){
            ?> <script>window.location = 'pindah_keluar.php?m=ubah'</script> <?php
        }else{ 
            ?> <script>window.location = 'pindah_keluar.php?m=gagal'</script> <?php
        }
    }
    ...
    if (isset($_POST['upfile2'])) {
        ...
        move_uploaded_file($namafile, '../img/'.$namabaru);
        $query = mysqli_query($kon, "UPDATE pindah_keluar SET file2 = '$namabaru' WHERE idpindahkeluar = '$idpindahkeluar'");
        if($query){
            ?> <script>window.location = 'pindah_keluar.php?m=ubah'</script> <?php
        }else{ 
            ?> <script>window.location = 'pindah_keluar.php?m=gagal'</script> <?php
    ...
    if (isset($_POST['upfile3'])) {
        ...
        move_uploaded_file($namafile, '../img/'.$namabaru);
        $query = mysqli_query($kon, "UPDATE pindah_keluar SET file3 = '$namabaru' WHERE idpindahkeluar = '$idpindahkeluar'");
        if($query){
            ?> <script>window.location = 'pindah_keluar.php?m=ubah'</script> <?php
        }else{ 
            ?> <script>window.location = 'pindah_keluar.php?m=gagal'</script> <?php
    ...
    $query = mysqli_query($kon, "UPDATE pindah_keluar SET tujuan = '$tujuan' WHERE idpindahkeluar = '$idpindahkeluar'");
      if($query){
      ?> <script>window.location = 'pindah_keluar.php?m=ubah'</script> <?php
      }else{ 
      ?> <script>window.location = 'pindah_keluar.php?m=gagal'</script> <?php
